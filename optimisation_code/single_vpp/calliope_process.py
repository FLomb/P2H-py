...
def netcdf_to_data(model_inst,reg):
    demand_dhw = model_inst.get_formatted_array('carrier_con').loc[{'techs':'demand_dhw', 'carriers':'dhw', 'locs':reg}].to_pandas().T
    hps_dhw = model_inst.get_formatted_array('carrier_prod').loc[{'techs':'hp_dhw', 'carriers':'dhw', 'locs':reg}].to_pandas().T
    hps_dhw_inputs = model_inst.get_formatted_array('carrier_con').loc[{'techs':'hp_dhw', 'carriers':'electricity', 'locs':reg}].to_pandas().T
    tes_dhw_out = model_inst.get_formatted_array('carrier_prod').loc[{'techs':'tes_dhw', 'carriers':'dhw', 'locs':reg}].to_pandas().T
    tes_dhw_in = model_inst.get_formatted_array('carrier_con').loc[{'techs':'tes_dhw', 'carriers':'dhw', 'locs':reg}].to_pandas().T
    tes_dhw_o = tes_dhw_out+tes_dhw_in
    tes_dhw_o[tes_dhw_o<0]=0
    tes_dhw_i = tes_dhw_out+tes_dhw_in
    tes_dhw_i[tes_dhw_i>0]=0
    tes_dhw_cap = model_inst.get_formatted_array('storage').loc[{'techs':'tes_dhw', 'locs':reg}].to_pandas().T
    tes_dhw_charge_min = model_inst.get_formatted_array('storage_cap_equals').loc[{'techs':'tes_dhw', 'locs':reg}].to_pandas().T * model_inst.get_formatted_array('storage_discharge_depth').loc[{'techs':'tes_dhw'}].values[0]
    
    grid = model_inst.get_formatted_array('carrier_prod').loc[{'techs':'grid', 'carriers':'electricity', 'locs':reg}].to_pandas().T
    pv_rooftop = model_inst.get_formatted_array('carrier_prod').loc[{'techs':'pv_rooftop', 'carriers':'electricity', 'locs':reg}].to_pandas().T
...
#    lgd1 = ax1.legstop(loc=0,  bbox_to_anchor=(1,1), fontsize='x-small')
...
#    lgd4 = ax2.legstop(loc=0,  bbox_to_anchor=(1,1), fontsize='x-small')
